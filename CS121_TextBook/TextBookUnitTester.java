 ...
 * 
 * @author mvail, marissa, lhindman, jerryfails
 */
public class TextBookUnitTester {
	...
	/**
	 * Confirms no exceptions are thrown when adding new posts and that postIDs.txt
	 * contains expected IDs.
	 */
	private static void testAddPost() {
		...
		try {
			...
			subtest = testName + " - " + TextBookInterface.POST_LIST_FILENAME;
			boolean subtestPassed = true;
			Scanner fileScan = new Scanner(new File(TextBookInterface.POST_LIST_FILENAME));
			for (int i = 1; i < 4; i++) {
				String line = fileScan.nextLine();
				int id = Integer.parseInt(line);
				if (id != i) {
					testResults += subTestFailure(subtest, "Line " + i + " value " + i, "Line " + i + " value " + id);
					testPassed = false;
					subtestPassed = false;
				}
			}
			if (fileScan.hasNext()) {
				testResults += subTestFailure(subtest, "No values after expected three IDs", fileScan.next());
				testPassed = false;
				subtestPassed = false;
			}
			if (subtestPassed) {
	...
	/**
	 * Confirm that removePost() returns the Post with expected values for valid 
	 * indexes and removes that Post's ID from postIDs.txt.
	 */
	private static void testRemovePost() {
		...
		try {
			...
			try {
				...
				testResults += subTestFailure(subtest, "removePost(-1) throws IndexOutOfBoundsException", "removePost(-1) returned: " + thePost);
				testPassed = false;					
			} catch (IndexOutOfBoundsException e) {
				testResults += subTestPass(subtest);
			} catch (Exception e) {
				testResults += subTestFailure(subtest, "IndexOutOfBoundsException", "Unexpected Exception");
				testResults += e.toString();
				testPassed = false;
				e.printStackTrace();
			}
			...
			try {
				...
				testResults += subTestFailure(subtest, "removePost(3) throws IndexOutOfBoundsException", "removePost(3) returned: " + thePost);
				testPassed = false;					
			} catch (IndexOutOfBoundsException e) {
				testResults += subTestPass(subtest);
			} catch (Exception e) {
			...
			try {
				...
				subtest = testName + " - " + TextBookInterface.POST_LIST_FILENAME + " after removePost(2) / id 2";
				subtestPassed = true;
				Scanner fileScan = new Scanner(new File(TextBookInterface.POST_LIST_FILENAME));
				String line = fileScan.nextLine();
				int id = Integer.parseInt(line);
				if (id != 1) {
					testResults += subTestFailure(subtest, "Line " + 1 + " value " + 1, "Line " + 1 + " value " + id);
					testPassed = false;
					subtestPassed = false;
				}
				...
				if (id != 2) {
					testResults += subTestFailure(subtest, "Line " + 2 + " value " + 2, "Line " + 2 + " value " + id);
					testPassed = false;
					subtestPassed = false;					
				}
				if (fileScan.hasNext()) {
					testResults += subTestFailure(subtest, "No values after expected two IDs", fileScan.next());
					testPassed = false;
					subtestPassed = false;
				}
				if (subtestPassed) {
			...
			try {
				...
				subtest = testName + " - " + TextBookInterface.POST_LIST_FILENAME + " after removePost(0) / id 1";
				subtestPassed = true;
				Scanner fileScan = new Scanner(new File(TextBookInterface.POST_LIST_FILENAME));
				String line = fileScan.nextLine();
				int id = Integer.parseInt(line);
				if (id != 2) {
					testResults += subTestFailure(subtest, "Line " + 1 + " value " + 2, "Line " + 1 + " value " + id);
					testPassed = false;
					subtestPassed = false;
				}
				if (fileScan.hasNext()) {
					testResults += subTestFailure(subtest, "No values after expected one ID", fileScan.next());
					testPassed = false;
					subtestPassed = false;
				}
				if (subtestPassed) {
			...
			try {
				...
				subtest = testName + " - " + TextBookInterface.POST_LIST_FILENAME + " after removePost(0) / id 2";
				subtestPassed = true;
				Scanner fileScan = new Scanner(new File(TextBookInterface.POST_LIST_FILENAME));
				if (fileScan.hasNext()) {
					testResults += subTestFailure(subtest, "No values after last remaining Post removed", fileScan.next());
					testPassed = false;
					subtestPassed = false;
				}
				if (subtestPassed) {
	...
	/**
	 * Confirm that addComment() throws no exceptions for Posts at valid indexes.
	 */
	private static void testAddComment() {
		...
		try {
			...
			try {
				subtest = testName + " - invalid index -1";
				theTextBook.addComment(-1, BOGUS, BOGUS);
				testResults += subTestFailure(subtest, "addComment(-1) throws IndexOutOfBoundsException", "addComment(-1) index accepted");
				testPassed = false;					
			} catch (IndexOutOfBoundsException e) {
				testResults += subTestPass(subtest);
			} catch (Exception e) {
			...
			try {
				subtest = testName + " - invalid index 3";
				theTextBook.addComment(3, BOGUS, BOGUS);
				testResults += subTestFailure(subtest, "addComment(3) throws IndexOutOfBoundsException", "addComment(3) index accepted");
				testPassed = false;					
			} catch (IndexOutOfBoundsException e) {
				testResults += subTestPass(subtest);
			} catch (Exception e) {
	...
	/**
	 * Confirm that lastID() returns the expected last ID after adding Posts to a
	 * fresh TextBook and after recovering a TextBook from files.
	 */
	...
	/**
	 * Confirm that getPostCount() returns expected counts after add and remove calls.
	 */
	...
	/**
	 * Confirm that getPostString() returns a String including the Post and all 
	 * comments for valid indexes.
	 */
	private static void testGetPostString() {
		...
		try {
			...
			try {
				...
				testResults += subTestFailure(subtest, "getPostString(-1) throws IndexOutOfBoundsException", "getPostString(-1) returned: " + postString);
				testPassed = false;					
			} catch (IndexOutOfBoundsException e) {
				testResults += subTestPass(subtest);
			} catch (Exception e) {
				testResults += subTestFailure(subtest, "IndexOutOfBoundsException", "Unexpected Exception thrown.");
				testResults += e.toString();
				testPassed = false;
				e.printStackTrace();
			}
			...
			try {
				...
				testResults += subTestFailure(subtest, "getPostString(3) throws IndexOutOfBoundsException", "getPostString(3) returned: " + postString);
				testPassed = false;					
			} catch (IndexOutOfBoundsException e) {
				testResults += subTestPass(subtest);
			} catch (Exception e) {
